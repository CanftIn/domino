...
std::optional<ReplacementItem> formatv_object_base::parseReplacementItem(
    StringRef Spec) {
...
formatv_object_base::splitLiteralAndReplacement(StringRef Fmt) {
  while (!Fmt.empty()) {
    ...
    if (RI) return std::make_pair(*RI, Right);
...
SmallVector<ReplacementItem, 2> formatv_object_base::parseFormatString(
    StringRef Fmt) {
  SmallVector<ReplacementItem, 2> Replacements;
  ReplacementItem I;
  while (!Fmt.empty()) {
    std::tie(I, Fmt) = splitLiteralAndReplacement(Fmt);
    if (I.Type != ReplacementType::Empty) Replacements.push_back(I);
  }
  return Replacements;
}
