#include <domino/support/Duration.h>
#include <domino/support/FileSystem.h>
#include <domino/support/Format.h>
#include <domino/support/FormatVariadic.h>
#include <domino/support/MathExtras.h>
...
raw_ostream::~raw_ostream() {
  ...
  if (BufferMode == BufferKind::InternalBuffer) delete[] OutBufStart;
}
...
raw_ostream &raw_ostream::write_hex(unsigned long long N) {
  domino::write_hex(*this, N, HexPrintStyle::Lower);
  return *this;
}
...
raw_ostream &raw_ostream::write_uuid(const uuid_t UUID) {
  for (int Idx = 0; Idx < 16; ++Idx) {
    *this << format("%02X", UUID[Idx]);
    if (Idx == 3 || Idx == 5 || Idx == 7 || Idx == 9) *this << "-";
  }
  return *this;
}
...
raw_ostream &raw_ostream::operator<<(const void *P) {
  domino::write_hex(*this, (uintptr_t)P, HexPrintStyle::PrefixLower);
  return *this;
}

raw_ostream &raw_ostream::operator<<(double N) {
  domino::write_double(*this, N, FloatStyle::Exponent);
  return *this;
}
...
raw_ostream &raw_ostream::write(unsigned char C) {
  ...
  if (DOMINO_UNLIKELY(OutBufCur >= OutBufEnd)) {
    if (DOMINO_UNLIKELY(!OutBufStart)) {
...
raw_ostream &raw_ostream::write(const char *Ptr, size_t Size) {
  // Group exceptional cases into a single branch.
  if (DOMINO_UNLIKELY(size_t(OutBufEnd - OutBufCur) < Size)) {
    if (DOMINO_UNLIKELY(!OutBufStart)) {
      if (BufferMode == BufferKind::Unbuffered) {
    ...
    if (DOMINO_UNLIKELY(OutBufCur == OutBufStart)) {
...
void raw_ostream::flush_tied_then_write(const char *Ptr, size_t Size) {
  if (TiedStream) TiedStream->flush();
  write_impl(Ptr, Size);
}
...
raw_ostream &raw_ostream::operator<<(const format_object_base &Fmt) {
  ...
  while (true) {
    ...
    if (BytesUsed <= NextBufferSize) return write(V.data(), BytesUsed);
...
raw_ostream &raw_ostream::operator<<(const FormattedNumber &FN) {
  if (FN.Hex) {
    ...
    domino::write_hex(*this, FN.HexValue, Style, FN.Width);
  } else {
    domino::SmallString<16> Buffer;
    domino::raw_svector_ostream Stream(Buffer);
    domino::write_integer(Stream, FN.DecValue, 0, IntegerStyle::Integer);
    if (Buffer.size() < FN.Width) indent(FN.Width - Buffer.size());
    (*this) << Buffer;
  }
  return *this;
}
...
raw_ostream &raw_ostream::operator<<(const FormattedBytes &FB) {
  ...
  if (FB.FirstByteOffset) {
    ...
    unsigned Power = 0;
    if (MaxOffset > 0) Power = domino::Log2_64_Ceil(MaxOffset);
    OffsetWidth = std::max<uint64_t>(4, domino::alignTo(Power, 4) / 4);
  }
  ...
  while (!Bytes.empty()) {
    ...
    if (FB.FirstByteOffset) {
      ...
      domino::write_hex(*this, Offset + LineIndex, HPS, OffsetWidth);
      *this << ": ";
    }
    ...
    for (size_t I = 0; I < Line.size(); ++I, CharsPrinted += 2) {
      ...
      domino::write_hex(*this, Line[I], HPS, 2);
    }
...
static raw_ostream &write_padding(raw_ostream &OS, unsigned NumChars) {
  ...
  if (NumChars < std::size(Chars)) return OS.write(Chars, NumChars);
...
bool raw_ostream::prepare_colors() {
  ...
  // terminal.
  if (sys::Process::ColorNeedsFlush() && !is_displayed()) return false;
...
raw_ostream &raw_ostream::changeColor(enum Colors colors, bool bold, bool bg) {
  ...
  if (colorcode) write(colorcode, strlen(colorcode));
  return *this;
}
...
raw_fd_ostream::raw_fd_ostream(int fd, bool shouldClose, bool unbuffered,
  ...
  // maintain this invariant. Many features have been added to DOMINO and clang
...
raw_fd_ostream::~raw_fd_ostream() {
  ...
  // TODO
  if (has_error()) DOMINO_ERROR_ABORT("IO failure on output stream");
}
...
void raw_fd_ostream::close() {
  ...
  ShouldClose = false;
  flush();
  if (auto EC = sys::Process::SafelyCloseFileDescriptor(FD)) error_detected(EC);
  FD = -1;
}
...
size_t raw_fd_ostream::preferred_buffer_size() const {
  ...
  // the complexity.
  if (S_ISCHR(statbuf.st_mode) && is_displayed()) return 0;
...
bool raw_fd_ostream::has_colors() const {
  if (!HasColors) HasColors = sys::Process::FileDescriptorHasColors(FD);
  return *HasColors;
}

Expected<sys::fs::FileLocker> raw_fd_ostream::lock() {
  std::error_code EC = sys::fs::lockFile(FD);
  if (!EC) return sys::fs::FileLocker(FD);
  return errorCodeToError(EC);
}

Expected<sys::fs::FileLocker> raw_fd_ostream::tryLockFor(
...
raw_fd_stream::raw_fd_stream(StringRef Filename, std::error_code &EC)
    : raw_fd_ostream(
          getFD(Filename, EC, sys::fs::CD_CreateAlways,
...
Error domino::writeToOutput(StringRef OutputFileName,
                            std::function<Error(raw_ostream &)> Write) {
  if (OutputFileName == "-") return Write(outs());
  ...
  if (!Temp) return createFileError(OutputFileName, Temp.takeError());
