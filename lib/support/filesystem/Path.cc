...
#include <domino/support/Signals.h>
...
using namespace domino::support::endian;

namespace {
using domino::StringRef;
using domino::sys::path::is_separator;
using domino::sys::path::Style;

inline Style real_style(Style style) {
  if (style != Style::native) return style;
  if (is_style_posix(style)) return Style::posix;
  return DOMINO_WINDOWS_PREFER_FORWARD_SLASH ? Style::windows_slash
...
inline char preferred_separator(Style style) {
  if (real_style(style) == Style::windows) return '\\';
  return '/';
}
...
size_t filename_pos(StringRef str, Style style) {
  ...
  if (is_style_windows(style)) {
    if (pos == StringRef::npos) pos = str.find_last_of(':', str.size() - 2);
  }
...
size_t root_dir_start(StringRef str, Style style) {
  ...
  // case "/"
  if (str.size() > 0 && is_separator(str[0], style)) return 0;
...
static std::error_code createUniqueEntity(
    const Twine &Model, int &ResultFD, SmallVectorImpl<char> &ResultPath,
    bool MakeAbsolute, FSEntity Type,
    sys::fs::OpenFlags Flags = sys::fs::OF_None, unsigned Mode = 0) {
  ...
  for (int Retries = 128; Retries > 0; --Retries) {
    ...
    switch (Type) {
      case FS_File: {
        ...
        if (EC) {
          // errc::permission_denied happens on Windows when we try to open a
          // file that has been marked for deletion.
      ...
      case FS_Name: {
        ...
        if (EC == errc::no_such_file_or_directory) return std::error_code();
        if (EC) return EC;
        continue;
      }
      ...
      case FS_Dir: {
        ...
        if (EC) {
          if (EC == errc::file_exists) continue;
          return EC;
        }
        return std::error_code();
      }
    }
    DOMINO_ERROR_ABORT("Invalid Type");
  }
  return EC;
}
...
void replace_extension(SmallVectorImpl<char> &path, const Twine &extension,
  ...
  // Append '.' if needed.
  if (ext.size() > 0 && ext[0] != '.') path.push_back('.');
...
static bool starts_with(StringRef Path, StringRef Prefix,
  ...
  if (is_style_windows(style)) {
    ...
    for (size_t I = 0, E = Prefix.size(); I != E; ++I) {
      ...
      if (SepPath != SepPrefix) return false;
      if (!SepPath && toLower(Path[I]) != toLower(Prefix[I])) return false;
    }
    return true;
  }
...
bool replace_path_prefix(SmallVectorImpl<char> &Path, StringRef OldPrefix,
  ...
  if (!starts_with(OrigPath, OldPrefix, style)) return false;

  // If prefixes have the same size we can simply copy the new one over.
  if (OldPrefix.size() == NewPrefix.size()) {
    domino::copy(NewPrefix, Path.begin());
    return true;
  }
...
void native(SmallVectorImpl<char> &Path, Style style) {
  ...
  if (is_style_windows(style)) {
    for (char &Ch : Path)
      if (is_separator(Ch, style)) Ch = preferred_separator(style);
...
StringRef stem(StringRef path, Style style) {
  ...
  if (pos == StringRef::npos) return fname;
...
bool is_absolute_gnu(const Twine &path, Style style) {
  ...
  // Handle '\\' on Windows.
  if (!p.empty() && is_separator(p.front(), style)) return true;
...
bool remove_dots(SmallVectorImpl<char> &the_path, bool remove_dot_dot,
  ...
  if (absolute) remaining = remaining.drop_front(root.size());
  ...
  while (!remaining.empty()) {
    ...
    if (next_slash == StringRef::npos) next_slash = remaining.size();
...
static std::error_code createTemporaryFile(
    const Twine &Model, int &ResultFD,
    domino::SmallVectorImpl<char> &ResultPath, FSEntity Type,
...
static std::error_code createTemporaryFile(
    const Twine &Prefix, StringRef Suffix, int &ResultFD,
    domino::SmallVectorImpl<char> &ResultPath, FSEntity Type,
    sys::fs::OpenFlags Flags = sys::fs::OF_None) {
...
std::error_code getPotentiallyUniqueFileName(
    const Twine &Model, SmallVectorImpl<char> &ResultPath) {
  int Dummy;
  return createUniqueEntity(Model, Dummy, ResultPath, false, FS_Name);
}

std::error_code getPotentiallyUniqueTempFileName(
    const Twine &Prefix, StringRef Suffix, SmallVectorImpl<char> &ResultPath) {
  int Dummy;
...
void make_absolute(const Twine &current_directory,
  ...
  // Already absolute.
  if ((rootName || is_style_posix(Style::native)) && rootDirectory) return;
...
std::error_code make_absolute(SmallVectorImpl<char> &path) {
  if (path::is_absolute(path)) return {};

  SmallString<128> current_dir;
  if (std::error_code ec = current_path(current_dir)) return ec;
...
std::error_code create_directories(const Twine &Path, bool IgnoreExisting,
  ...
  // return it.
  if (EC != errc::no_such_file_or_directory) return EC;

  // We failed because of a no_such_file_or_directory, try to create the
  // parent.
  StringRef Parent = path::parent_path(P);
  if (Parent.empty()) return EC;

  if ((EC = create_directories(Parent, IgnoreExisting, Perms))) return EC;
...
std::error_code copy_file(const Twine &From, const Twine &To) {
  int ReadFD, WriteFD;
  if (std::error_code EC = openFileForRead(From, ReadFD, OF_None)) return EC;
...
ErrorOr<MD5::MD5Result> md5_contents(int FD) {
  ...
  if (BytesRead < 0) return std::error_code(errno, std::generic_category());
...
ErrorOr<MD5::MD5Result> md5_contents(const Twine &Path) {
  int FD;
  if (auto EC = openFileForRead(Path, FD, OF_None)) return EC;
...
file_type get_file_type(const Twine &Path, bool Follow) {
  file_status st;
  if (status(Path, st, Follow)) return file_type::status_error;
  return st.type();
}
...
std::error_code is_directory(const Twine &path, bool &result) {
  file_status st;
  if (std::error_code ec = status(path, st)) return ec;
...
std::error_code is_symlink_file(const Twine &path, bool &result) {
  file_status st;
  if (std::error_code ec = status(path, st, false)) return ec;
...
std::error_code is_other(const Twine &Path, bool &Result) {
  ...
  if (std::error_code EC = status(Path, FileStatus)) return EC;
...
ErrorOr<perms> getPermissions(const Twine &Path) {
  file_status Status;
  if (std::error_code EC = status(Path, Status)) return EC;
...
Error readNativeFileToEOF(file_t FileHandle, SmallVectorImpl<char> &Buffer,
  ...
  for (;;) {
    ...
    if (!ReadBytes) return ReadBytes.takeError();
    if (*ReadBytes == 0) return Error::success();
    Size += *ReadBytes;
  }
}
...
std::string getMainExecutable(const char *Argv0, void *MainAddr) {
  if (IsLLVMDriver) return sys::path::stem(Argv0).str();
