...
static inline uint32_t blk(uint32_t *Buf, int I) {
  Buf[I & 15] = rol(
      Buf[(I + 13) & 15] ^ Buf[(I + 8) & 15] ^ Buf[(I + 2) & 15] ^ Buf[I & 15],
      1);
  return Buf[I & 15];
}
...
void SHA1::update(ArrayRef<uint8_t> Data) {
  ...
  if (InternalState.BufferOffset > 0) {
    ...
    for (size_t I = 0; I < Remainder; ++I) addUncounted(Data[I]);
  ...
  for (uint8_t C : Data) addUncounted(C);
}
...
void SHA1::pad() {
  ...
  addUncounted(0x80);
  while (InternalState.BufferOffset != 56) addUncounted(0x00);
