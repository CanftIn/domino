...
class MLIRGenImpl {
 ...
 private:
  ...
  domino::script::FuncOp mlirGen(PrototypeAST &proto) {
    ...
    return builder.create<domino::script::FuncOp>(location, proto.getName(),
                                                  funcType);
  }

  /// Emit a new function and add it to the MLIR module.
  domino::script::FuncOp mlirGen(FunctionAST &funcAST) {
    // Create a scope in the symbol table to hold variable declarations.
    ScopedHashTableScope<::llvm::StringRef, mlir::Value> varScope(symbolTable);

    // Create an MLIR function for the given prototype.
    builder.setInsertionPointToEnd(theModule.getBody());
    domino::script::FuncOp function = mlirGen(*funcAST.getProto());
