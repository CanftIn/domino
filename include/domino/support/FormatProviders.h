#ifndef DOMINO_SUPPORT_FORMATPROVIDER_H_
#define DOMINO_SUPPORT_FORMATPROVIDER_H_
...
struct use_string_formatter
    : public std::integral_constant<
          bool, std::is_convertible<T, domino::StringRef>::value> {};
...
class HelperFunctions {
 protected:
  ...
  static bool consumeHexStyle(StringRef &Str, HexPrintStyle &Style) {
    if (!Str.starts_with_insensitive("x")) return false;
  ...
  static size_t consumeNumHexDigits(StringRef &Str, HexPrintStyle Style,
    ...
    if (isPrefixedHexStyle(Style)) Default += 2;
    return Default;
  }
};
}  // namespace detail
...
struct format_provider<
 ...
 private:
 public:
  static void format(const T &V, domino::raw_ostream &Stream, StringRef Style) {
    HexPrintStyle HS;
    size_t Digits = 0;
...
/// objects such as std::string and domino::StringRef.
///
...
struct format_provider<T,
  ...
  static void format(const char &V, domino::raw_ostream &Stream,
                     StringRef Style) {
    if (Style.empty())
      Stream << V;
    else {
...
struct format_provider<bool> {
  static void format(const bool &B, domino::raw_ostream &Stream,
                     StringRef Style) {
...
struct format_provider<T,
  ...
  static void format(const T &V, domino::raw_ostream &Stream, StringRef Style) {
    ...
    if (!Precision) Precision = getDefaultPrecision(S);
...
template <typename IterT>
class format_provider<domino::iterator_range<IterT>> {
  ...
  static StringRef consumeOneOption(StringRef &Style, char Indicator,
                                    StringRef Default) {
    if (Style.empty()) return Default;
    if (Style.front() != Indicator) return Default;
 ...
 public:
  ...
  static void format(const domino::iterator_range<IterT> &V,
                     domino::raw_ostream &Stream, StringRef Style) {
    StringRef Sep;
    StringRef ArgStyle;
...
#endif  // DOMINO_SUPPORT_FORMATPROVIDER_H_
