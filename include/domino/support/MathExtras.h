#ifndef DOMINO_SUPPORT_MATHEXTRAS_H_
#define DOMINO_SUPPORT_MATHEXTRAS_H_
...
constexpr double e = 2.7182818284590452354,  // (0x1.5bf0a8b145749P+1)
    ...
    sqrt2 = 1.4142135623730950488,           // (0x1.6a09e667f3bcdP+0)
                                             // https://oeis.org/A00219
...
T findLastSet(T Val, ZeroBehavior ZB = ZB_Max) {
  ...
  // Use ^ instead of - because both gcc and domino can remove the associated ^
  // in the __builtin_clz intrinsic on x86.
  return domino::countl_zero(Val) ^ (std::numeric_limits<T>::digits - 1);
}
...
T reverseBits(T Val) {
  ...
  if constexpr (std::is_same_v<T, uint8_t>) return __builtin_bitreverse8(Val);
#endif
#if __has_builtin(__builtin_bitreverse16)
  if constexpr (std::is_same_v<T, uint16_t>) return __builtin_bitreverse16(Val);
#endif
#if __has_builtin(__builtin_bitreverse32)
  if constexpr (std::is_same_v<T, uint32_t>) return __builtin_bitreverse32(Val);
#endif
#if __has_builtin(__builtin_bitreverse64)
  if constexpr (std::is_same_v<T, uint64_t>) return __builtin_bitreverse64(Val);
#endif
...
constexpr inline bool isInt(int64_t x) {
  if constexpr (N == 8) return static_cast<int8_t>(x) == x;
  if constexpr (N == 16) return static_cast<int16_t>(x) == x;
  if constexpr (N == 32) return static_cast<int32_t>(x) == x;
  if constexpr (N < 64)
...
constexpr inline bool isUInt(uint64_t x) {
  ...
  if constexpr (N == 8) return static_cast<uint8_t>(x) == x;
  if constexpr (N == 16) return static_cast<uint16_t>(x) == x;
  if constexpr (N == 32) return static_cast<uint32_t>(x) == x;
  if constexpr (N < 64) return x < (UINT64_C(1) << (N));
  (void)x;  // MSVC v19.25 warns that x is unused.
  return true;
}
...
constexpr inline bool isPowerOf2_32(uint32_t Value) {
  return domino::has_single_bit(Value);
}
...
unsigned countLeadingOnes(T Value) {
  static_assert(std::is_unsigned_v<T>,
                "Only unsigned integral types are allowed.");
  return domino::countl_one<T>(Value);
}
...
unsigned countTrailingOnes(T Value) {
  static_assert(std::is_unsigned_v<T>,
                "Only unsigned integral types are allowed.");
  return domino::countr_one<T>(Value);
}
...
inline unsigned countPopulation(T Value) {
  static_assert(std::is_unsigned_v<T>,
                "Only unsigned integral types are allowed.");
  return (unsigned)domino::popcount(Value);
}
...
inline bool isShiftedMask_32(uint32_t Value, unsigned &MaskIdx,
                             unsigned &MaskLen) {
  if (!isShiftedMask_32(Value)) return false;
  MaskIdx = domino::countr_zero(Value);
  MaskLen = domino::popcount(Value);
  return true;
}
...
inline bool isShiftedMask_64(uint64_t Value, unsigned &MaskIdx,
                             unsigned &MaskLen) {
  if (!isShiftedMask_64(Value)) return false;
  MaskIdx = domino::countr_zero(Value);
  MaskLen = domino::popcount(Value);
  return true;
}
...
constexpr inline size_t CTLog2() {
  static_assert(kValue > 0 && domino::isPowerOf2_64(kValue),
...
inline unsigned Log2_32(uint32_t Value) {
  return 31 - domino::countl_zero(Value);
}

/// Return the floor log base 2 of the specified value, -1 if the value is zero.
/// (64 bit edition.)
inline unsigned Log2_64(uint64_t Value) {
  return 63 - domino::countl_zero(Value);
}
...
inline unsigned Log2_32_Ceil(uint32_t Value) {
  return 32 - domino::countl_zero(Value - 1);
}

/// Return the ceil log base 2 of the specified value, 64 if the value is zero.
/// (64 bit edition.)
inline unsigned Log2_64_Ceil(uint64_t Value) {
  return 64 - domino::countl_zero(Value - 1);
}

/// This function takes a 64-bit integer and returns the bit equivalent double.
inline double BitsToDouble(uint64_t Bits) {
  static_assert(sizeof(uint64_t) == sizeof(double), "Unexpected type sizes");
  return domino::bit_cast<double>(Bits);
}

/// This function takes a 32-bit integer and returns the bit equivalent float.
inline float BitsToFloat(uint32_t Bits) {
  static_assert(sizeof(uint32_t) == sizeof(float), "Unexpected type sizes");
  return domino::bit_cast<float>(Bits);
}
...
inline uint64_t DoubleToBits(double Double) {
  static_assert(sizeof(uint64_t) == sizeof(double), "Unexpected type sizes");
  return domino::bit_cast<uint64_t>(Double);
}
...
inline uint32_t FloatToBits(float Float) {
  static_assert(sizeof(uint32_t) == sizeof(float), "Unexpected type sizes");
  return domino::bit_cast<uint32_t>(Float);
}
...
inline uint64_t PowerOf2Floor(uint64_t A) { return domino::bit_floor(A); }
...
template <typename T>
std::enable_if_t<std::is_unsigned<T>::value, T> SaturatingAdd(
...
template <typename T>
std::enable_if_t<std::is_unsigned<T>::value, T> SaturatingMultiplyAdd(
...
#endif  // DOMINO_SUPPORT_MATHEXTRAS_H_