#ifndef DOMINO_SUPPORT_FILESYSTEM_H_
#define DOMINO_SUPPORT_FILESYSTEM_H_

#include <domino/support/Chrono.h>
#include <domino/support/ErrorOr.h>
#include <domino/support/Expected.h>
...
/// @returns errc::success if \a Name has been opened, otherwise a
///          platform-specific error_code.
inline std::error_code openFileForWrite(
    const Twine &Name, int &ResultFD,
    CreationDisposition Disp = CD_CreateAlways, OpenFlags Flags = OF_None,
    unsigned Mode = 0666) {
...
class recursive_directory_iterator {
 ...
 public:
  ...
  void pop() {
    ...
    std::error_code ec;
    do {
      if (ec) DOMINO_ERROR_ABORT("Error incrementing directory iterator.");
      State->Stack.pop();
      --State->Level;
    } while (!State->Stack.empty() &&
             State->Stack.top().increment(ec) == end_itr);
...
#endif  // DOMINO_SUPPORT_FILESYSTEM_H_