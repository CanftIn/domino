...
template <>
struct CastInfo<T2, T1> : public OptionalValueCast<T2, T1> {};
...
template <>
struct CastInfo<T3, T1 *> : public ValueFromPointerCast<T3, T1> {};
...
template <>
struct ValueIsPresent<const pointer_wrappers::PTy> {
  using UnwrappedType = pointer_wrappers::PTy;
  static inline bool isPresent(const pointer_wrappers::PTy &P) {
    return P.get() != nullptr;
  }
...
template <>
struct simplify_type<pointer_wrappers::PTy> {
...
template <>
struct simplify_type<const pointer_wrappers::PTy> {
  typedef pointer_wrappers::Base *SimpleType;
