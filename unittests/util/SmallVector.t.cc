...
TEST(SmallVectorTest, MidInsert) {
  ...
  for (MovedFrom& m : v) EXPECT_TRUE(m.hasValue);
}
...
class SmallVectorReferenceInvalidationTest : public SmallVectorTestBase {
 protected:
  ...
  void SetUp() override {
    ...
    for (int I = 0, E = NumBuiltinElts(V); I != E; ++I) V.emplace_back(I + 1);
  }
};
...
TYPED_TEST(SmallVectorReferenceInvalidationTest, PushBack) {
  ...
  // Fill storage again.
  V.back() = V.size();
  while (V.size() < V.capacity()) V.push_back(V.size() + 1);
...
TYPED_TEST(SmallVectorInternalReferenceInvalidationTest, EmplaceBack) {
  ...
  while (V.size() < V.capacity()) V.emplace_back(V.size() + 1, V.size() + 1);
